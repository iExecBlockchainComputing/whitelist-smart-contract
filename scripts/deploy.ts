...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (deploymentResult.newlyDeployed) {
    console.log(`IExecWhitelist deployed at address: ${deploymentResult.address}`);
    console.log(`Transaction hash: ${deploymentResult.receipt?.transactionHash}`);
    ...
    const receipt = await ethers.provider.getTransactionReceipt(transactionHash as string);
    ...
    console.log("IExecWhitelist already deployed at address:", deploymentResult.address);
  }
};
